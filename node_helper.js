...
module.exports = NodeHelper.create({
  ...
  async _fetchTemperatureData() {
    ...
    await Promise.all(
      ...
      console.error(
        "[MMM-RemoteTemperature] Unhandled promise rejection:",
        error.message,
      );
    });
    ...
    if (deviceCount > 0) {
      averageTemperature = this._roundToTwoDecimalPlaces(
        totalTemperature / deviceCount,
      );
    }
    ...
    this.sendSocketNotification(
      "MMM-RemoteTemperature.VALUE_RECEIVED",
      this.viewModel,
    );
  },
